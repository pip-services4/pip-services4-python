...
from test.containers.DummyCommandableLambdaFunction import DummyCommandableLambdaFunction

DUMMY1: Dummy = Dummy(None, 'Key 1', 'Content 1')
DUMMY2: Dummy = Dummy(None, 'Key 2', 'Content 2')


class TestDummyCommandableLambdaFunction:
    lambda_func: DummyCommandableLambdaFunction = None

    @classmethod
    def setup_class(cls):
        config = ConfigParams.from_tuples(
            'logger.descriptor', 'pip-services:logger:console:default:1.0',
            'controller.descriptor', 'pip-services-dummies:service:default:default:1.0',
        )

        cls.lambda_func = DummyCommandableLambdaFunction()
    ...
    def tests_crud_operations(self):
        # Create one dummy
