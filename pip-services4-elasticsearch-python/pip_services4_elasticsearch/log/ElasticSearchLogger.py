...
class ElasticSearchLogger(CachedLogger, IReferenceable, IOpenable):
    ...
    .. code-block:: python
        ...
        try:
            ...
            logger.error(context, err, "Error occured: {}", err.message)
            # do something

        logger.debug(context, "Everything is OK.");
    """
    ...
    def open(self, context: Optional[IContext]):
        ...
        try:
            self.__create_index_if_needed(context, True)
    ...
    def __create_index_if_needed(self, context: Optional[IContext], force):
    ...
    def _save(self, messages: List[LogMessage]):
        ...
        if not self.is_open() and len(messages) == 0:
            return
        try:
            self.__create_index_if_needed(Context.from_trace_id('elasticsearch_logger'), False)
            bulk = []
