...
from pip_services4_sqlserver.persistence.SqlServerPersistence import SqlServerPersistence
...
class IdentifiableSqlServerPersistence(SqlServerPersistence):
    ...
    def set(self, context: Optional[IContext], item: T) -> Optional[T]:
        ...
        if new_item is not None:
            self._logger.trace(context, "Set in %s with id = %s", self._table_name, item['id'])
    ...
    def update(self, context: Optional[IContext], item: T) -> Optional[T]:
        ...
        self._logger.trace(context, "Updated in %s with id = %s", self._table_name, item.id)
